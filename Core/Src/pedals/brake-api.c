...
EAGLETRT_STATIC enum BrakeReturnCode brake_error_status = BRAKE_RC_NO_ERROR;
...
float brake_get_travel_percentage() {
	...
	if(return_code == VOLTAGE_SCALING_RC_ERROR){
		brake_error_status = BRAKE_RC_CALLBACK_FAILURE;
		return 0.0F;
	}
	if (res < 0.0F) {
		brake_error_status = BRAKE_RC_VALUE_OUTSIDE_RANGE;
		return 0.0F;
	} 
	if (res > 1.0F) {
		brake_error_status = BRAKE_RC_VALUE_OUTSIDE_RANGE;
...
enum BrakeReturnCode brake_get_error_status() {
	enum BrakeReturnCode error_state = brake_error_status;
	brake_error_status = BRAKE_RC_NO_ERROR;
	return error_state;
}
