...
void test_brake_bpps_percentage(void) {
	const int32_t MIN_VAL = BPPS_MIN_VALUE, MAX_VAL = BPPS_MAX_VALUE, CURRENT = 4095;
	int32_t sensor = MIN_VAL + (MAX_VAL - MIN_VAL) / 2;

	insert_values_in_array(CURRENT,0,0,sensor,0,0,0);

	float res = brake_get_percentage();
...
void test_brake_front_pressure() {
	...
	TEST_ASSERT_FLOAT_WITHIN(0.001 * BSPS_F_CONVERSION_VALUE, 0.50f * BSPS_F_CONVERSION_VALUE, res); // [49.9%,50.1%] range is valid
}
...
void test_brake_rear_pressure() {
	...
	TEST_ASSERT_FLOAT_WITHIN(0.001 * BSPS_R_CONVERSION_VALUE, 0.50f * BSPS_R_CONVERSION_VALUE, res); // [49.85%,50.15%] range is valid
}
